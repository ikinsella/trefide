...
int l1tf(const int n, const double *y, const double lambda, double *x,
         const double rescale, const int MAXITER, const int verbose)
{
    /* parameters */
    ...
    for (iters = 0; iters <= MAXITER; iters++)
    {
        ...
        pobj1 = rescale*F77_CALL(ddot)(&m,w,&ione,tmp_m1,&ione)
                +lambda*(vecsum(m,mu1)+vecsum(m,mu2));
        pobj2 = rescale*zTDDTz + lambda*F77_CALL(dasum)(&m,tmp_m2,&ione);
        pobj  = min(pobj1,pobj2);
        dobj  =-rescale*zTDDTz+F77_CALL(ddot)(&m,Dy,&ione,z,&ione);
