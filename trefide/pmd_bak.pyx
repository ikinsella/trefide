...
cdef size_t do_something_else(const size_t d1, 
                        const size_t d2, 
                        const size_t t,
                        double* R, 
                        double* U,
    ...
    cdef size_t i, j, k
    for i in range(d1*d2*t):
        R[i] = 3.0
    for j in range(d1*d2*max_components):
        U[j] -= 1
    for k in range(t*max_components):
        V[k] *=9
    return 1


cpdef size_t do_something(const size_t d1, 
                          const size_t d2, 
                          const size_t t,
                          double[::1] Y, 
                          double[::1] U,
    ...
    with a penalized matrix decomposition
    """ 
    return do_something_else(d1, d2, t, &Y[0], &U[0], &V[0], lambda_tv, 
                        spatial_thresh, max_components, max_iters, tol)
# -----------------------------------------------------------------------------#
...
cpdef block_pmd(const size_t d1, 
    ...
    with nogil, parallel(num_threads=2):
        for b in prange(num_blocks, schedule="guided"):
             K[b] = do_something(bheight, bwidth, t, 
                             R[b,:], U[b,:], V[b,:],
                             lambda_tv, spatial_thresh, 
                             max_components, max_iters,tol)
             #K[b] = call_pmd(bheight, bwidth, t, 
             #                R[b,:], U[b,:], V[b,:],
             #                lambda_tv, spatial_thresh, 
             #                max_components, max_iters,tol)
