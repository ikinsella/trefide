...
int l1tf(const int n, const double *y, const double lambda, double *x,
    ...
    if (info > 0) /* if Cholesky factorization fails, try LU factorization */
    {
        ...
        F77_CALL(dgbtrf)
        (&m, &m, &itwo, &itwo, DDTF, &iseven, IPIV, &info);
    }
    ...
    for (*iter = 1; *iter <= maxiter; (*iter)++) {
        ...
        F77_CALL(dcopy)
        (&m, Dy, &ione, w, &ione); /* w = D*y-(mu1-mu2)  */
        F77_CALL(daxpy)
        (&m, &dminusone, mu1, &ione, w, &ione);
        F77_CALL(daxpy)
        (&m, &done, mu2, &ione, w, &ione);

        F77_CALL(dcopy)
        (&m, Dy, &ione, tmp_m2, &ione); /* tmp_m2 = D*y-D*D'*z */
        F77_CALL(daxpy)
        (&m, &dminusone, DDTz, &ione, tmp_m2, &ione);

        F77_CALL(dcopy)
        (&m, w, &ione, tmp_m1, &ione); /* tmp_m1 = w*(D*D')^-1*w */
        if (ddtf_chol == TRUE) {
            F77_CALL(dpbtrs)
            ("L", &m, &itwo, &ione, DDTF, &ithree, tmp_m1, &m, &info);
        } else {
            F77_CALL(dgbtrs)
            ("N", &m, &itwo, &itwo, &ione, DDTF, &iseven, IPIV, tmp_m1, &m,
             &info);
        }
        ...
        F77_CALL(dcopy)
        (&m, DDTz, &ione, rz, &ione); /* rz = D*D'*z-w */
        F77_CALL(daxpy)
        (&m, &dminusone, w, &ione, rz, &ione);

        yainvx(m, +1.0 / t, f1,
               dz); /* dz = r = D*y-D*D'*z+(1/t)./f1-(1/t)./f2 */
        yainvx(m, -1.0 / t, f2, tmp_m1);
        F77_CALL(daxpy)
        (&m, &done, tmp_m1, &ione, dz, &ione);
        F77_CALL(daxpy)
        (&m, &done, tmp_m2, &ione, dz, &ione);
        ...
        F77_CALL(dpbsv)
        ("L", &m, &itwo, &ione, S, &ithree, dz, &m, &info); /* dz=S\r */
        ...
        for (i = 0; i < m; i++) {
            if (dmu1[i] < 0 && -mu1[i] / dmu1[i] < ratio)
                ratio = -mu1[i] / dmu1[i];
            if (dmu2[i] < 0 && -mu2[i] / dmu2[i] < ratio)
                ratio = -mu2[i] / dmu2[i];
        }
        ...
        F77_CALL(daxpy)
        (&m, &step, dz, &ione, z, &ione);
        F77_CALL(daxpy)
        (&m, &step, dmu1, &ione, mu1, &ione);
        F77_CALL(daxpy)
        (&m, &step, dmu2, &ione, mu2, &ione);
        ...
        for (lsiters = 0; lsiters < MAXLSITER; lsiters++) {
            ...
            if (linesearch_skip != 1) {
                ...
                (&m, DDTz, &ione, rz, &ione);
                F77_CALL(daxpy)
                (&m, &dminusone, Dy, &ione, rz, &ione);
                F77_CALL(daxpy)
                (&m, &done, mu1, &ione, rz, &ione);
                F77_CALL(daxpy)
                (&m, &dminusone, mu2, &ione, rz, &ione);
            ...
            F77_CALL(daxpy)
            (&m, &diff_step, dz, &ione, z, &ione);
            F77_CALL(daxpy)
            (&m, &diff_step, dmu1, &ione, mu1, &ione);
            F77_CALL(daxpy)
            (&m, &diff_step, dmu2, &ione, mu2, &ione);
            step *= BETA;
        }
    }
    if (verbose) {
...
double l1tf_lambdamax(const int n, double *y, const int verbose) {
    ...
    F77_CALL(dpbsv)
    ("L", &m, &itwo, &ione, mat, &ithree, vec, &m, &info);
    if (info > 0) /* if Cholesky factorization fails, try LU factorization */
    {
        if (verbose) {
        ...
        F77_CALL(dgbsv)
        (&m, &itwo, &itwo, &ione, mat, &iseven, piv, vec, &m, &info);
        if (info > 0) {
            free(vec);
            free(mat);
            free(piv);
