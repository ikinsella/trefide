...
short weighted_pdas(const int n,          // data length
    ...
    for (*iter = 1; *iter <= maxiter; (*iter)++) {
        ...
        n_vio = locate_violators(n, z, lambda, diff_x, vio_index, vio_fitness,
                                 vio_sort);
        ...
        std::sort(vio_sort, vio_sort + n_vio, FitnessComparator(vio_fitness));
...
*/
void update_primal(const int n, double *x, const double *y, const double *wi,
